...
- Politely decline requests for information not available on WHO's website"""
    ...
    def _build_prompt(
        ...
        if charts:
            prompt_parts.append("\n## 📊 Charts Available for Report (FROM DASHBOARD/INTERACTIVE CHART SYSTEM):")
            prompt_parts.append("\nIMPORTANT: These charts are generated using the SAME data and chart generators used in the Dashboard and Interactive Charts sections.")
            prompt_parts.append("They represent actual visualizations from the system, not hypothetical charts.")
            ...
            prompt_parts.append("These charts show the ACTUAL data patterns from the dashboard visualizations.")
        ...
        prompt_parts.append("   - Present the key statistics from the dashboard data")
        ...
        prompt_parts.append("   - Reference relevant dashboard charts using [CHART: name] notation")
        prompt_parts.append("   - Interpret what the charts show (trends, patterns, comparisons)")
        prompt_parts.append("4. Visual Analytics Section - Describe key insights from the dashboard charts")
        prompt_parts.append("   - Reference each chart by name")
        prompt_parts.append("   - Explain what patterns the chart reveals")
        prompt_parts.append("   - Connect chart insights to the data analysis")
        prompt_parts.append("5. Trends and Patterns - Explain what dashboard visualizations indicate FOR SELECTED INDICATORS ONLY")
        prompt_parts.append("6. Performance Assessment - Against WHO targets (as shown in charts)")
        prompt_parts.append("7. Recommendations - Actionable insights based on the SELECTED INDICATORS and chart patterns")
        ...
        prompt_parts.append("- Use the DASHBOARD DATA and CHARTS provided above")
        prompt_parts.append("- Charts are from the actual Dashboard/Interactive Charts system - reference them specifically")
        ...
        prompt_parts.append("  * Reference to relevant dashboard chart showing this pattern")
        ...
        prompt_parts.append("- Integrate chart references throughout the report using [CHART: name]")
        prompt_parts.append("- Explain what each dashboard chart reveals about the data")
        prompt_parts.append("- Structure the report with markdown headings (##, ###)")
        prompt_parts.append("- Be specific and data-driven, using dashboard visualizations as evidence")
